...
var queryString = function queryString(item, config) {
  ...
  if (type === 'group' && children && children.size) {
    ...
    var value = properties.get('value').map(function (currentValue, ind) {
      ...
      if (valueSrc == 'field') {
        ...
        if (rightField) {
          ...
          var _fieldParts = rightField.split(fieldSeparator); //let fieldKeys = getFieldPath(rightField, config);
        ...
        if (typeof fieldWidgetDefinition.formatValue === 'function') {
          ...
          var _args = [currentValue, (0, _pick.default)(fieldDefinition, ['fieldSettings', 'listValues']), (0, _omit.default)(fieldWidgetDefinition, ['formatValue', 'mongoFormatValue']), //useful options: valueFormat for date/time
          isForDisplay];
    ...
    var fieldParts = field.split(fieldSeparator); //let fieldKeys = getFieldPath(field, config);
    ...
    var formattedField = config.settings.formatField(field, fieldParts, fieldLabel2, fieldDefinition, config, isForDisplay); //format expr

    var args = [formattedField, operator, formattedValue, valueSrcs.length > 1 ? valueSrcs : valueSrcs[0], valueTypes.length > 1 ? valueTypes : valueTypes[0], (0, _omit.default)(operatorDefinition, ['formatOp', 'mongoFormatOp']), operatorOptions, isForDisplay];
